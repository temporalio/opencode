...
describe("WorktreeMaterializer", () => {
  ...
  // The write direction. A host the store has moved past must not pack its older files: by time it
  // would become the newest, and every other host would check that out over the work they were
  // shipped to carry. The same rule as the read direction, applied where it was missing.
  ...
  // only a rebuild writes one. Gating the move on that marker would fail every activity such a host
  // draws as soon as any other host ships. The note is the rule: a host that agreed to a state may
  // be moved off it.
