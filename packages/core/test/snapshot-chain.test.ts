...
// wrote the row, and hosts do not agree on the time, so ordering by it would let a worker whose
// clock is behind make its older tree the newest one that every other host then checks out.
