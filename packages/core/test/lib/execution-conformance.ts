...
export const runContract = (label: string, makeExec: ReturnType<typeof makeExecutionFor>) => {
  ...
  describe(`SessionExecution contract: ${label}`, () => {
...

    {
      ...
      it.live(
        "a tool call runs once and the turn goes back to the model with its result",
        () =>
          withIdleOverride(
            Effect.gen(function* () {
              yield* seedSession(sessionID)
              ...
              const store = yield* SessionStore.Service
              const context = yield* store.context(sessionID)
              const parts = context.flatMap((message) => (message.type === "assistant" ? message.content : []))
              const call = parts.findLast(
                (part): part is SessionMessage.AssistantTool => part.type === "tool" && part.id === "call_contract",
              )
...

    {
      ...
      it.live(
        "closes a tool call the interrupt cut short",
        () =>
          withIdleOverride(
            Effect.gen(function* () {
              yield* seedSession(sessionID)
