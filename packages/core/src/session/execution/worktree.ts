...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const materialize = Effect.fnUntraced(function* (tip: typeof SnapshotPackTable.$inferSelect, fromEmpty = false) {
      ...
      yield* proc
        .run(
          ChildProcess.make("git", ["--git-dir", repository.gitDirectory, "update-ref", RESTORED, tip.id], {
            cwd: worktree,
            extendEnv: true,
          }),
        )
      ...
      yield* proc
        .run(
          ChildProcess.make("git", ["--git-dir", repository.gitDirectory, "symbolic-ref", "HEAD", RESTORED], {
            cwd: worktree,
            extendEnv: true,
          }),
        )
    ...
    const ensure = Effect.fn("WorktreeMaterializer.ensure")(function* (
      ...
      // captured the tree from here. Moving it forward from a state it agreed to loses nothing. The
      // rebuild marker is not part of the rule: a host that seeded the session from its own checkout
      // has a note but no marker, and gating on the marker would refuse every activity it draws once
      // any other host ships. A developer's checkout is protected by having no note at all.
            ...
            Effect.catchCauseIf(
              ...
              (cause) =>
                Effect.gen(function* () {
                  ...
                  if (!here) yield* Effect.promise(() => rm(tip.worktree, { recursive: true, force: true }))
