...
export const orderChain = <T extends ChainRow>(rows: readonly T[]): T[] => {
  ...
  return [...rows].sort((a, b) => (depth.get(a.id) ?? 0) - (depth.get(b.id) ?? 0) || a.time_created - b.time_created)
}

/** The newest state the store holds, which is the deepest link in the chain. */
export const chainHead = <T extends ChainRow>(rows: readonly T[], depth = depths(rows)): T | undefined => {
 ...
 */
export const isBehind = <T extends ChainRow & { readonly tree: string }>(rows: readonly T[], tree: string): boolean => {
  const depth = depths(rows)
  const head = chainHead(rows, depth)
