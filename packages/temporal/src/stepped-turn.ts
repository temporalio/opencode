...
export const isHaltFailure = (error: unknown) =>
  error instanceof ActivityFailure && (error.cause as ApplicationFailure | undefined)?.type === HALTED_FAILURE_TYPE
...
export const makeSteppedTurn =
  ({ activities, isCancellation, isHalt, log, serial, nonCancellable, pinnedTo }: SteppedTurnDeps) =>
  async (input: StepDrainInput): Promise<StepDrainResult> => {
    ...
    for (const outcome of dispatched) {
      ...
      if (isCancellation(outcome.reason) || isHalt(outcome.reason)) {
        ...
        //
        // Explicitly with no continuation. A seal left to decide re-derives "keep going" from the
        // tool parts and would carry the agent on past a declined permission. The reason the turn
        // is stopping is rethrown either way, and a seal that fails here must not replace it.
        await (nonCancellable ?? ((fn: () => Promise<unknown>) => fn()))(() => seal(true)).catch((err) =>
          log?.("could not seal an interrupted step", { step: model.step, error: String(err) }),
        )
