...
const turnEndingReasons = ["CANCELLED", "NOT_FOUND"]

// The event-log owner token for one activity execution. A retry of the same step gets a fresh
// attempt, so once it claims the log the attempt before it is fenced out. The activity id is what
// keeps steps apart: attempt numbers restart at 1 on every step, so a token without it repeats.
...
const withHeartbeat = async <A>(body: () => Promise<A>): Promise<A> => {
...
export function makeStepActivities(
  ...
  return {
    async runTurnStep(input) {
      return withHeartbeat(() => stepDrain({ ...input, owner: ownerToken() }, Context.current().cancellationSignal))
    },
  }
}
  ...
  return {
    ...
    async runToolCall(input) {
      ...
      const turnEnded = () => {
        ...
        return reason instanceof CancelledFailure && turnEndingReasons.includes(reason.message ?? "")
      }
      return withHeartbeat(() => drains.toolCallDrain(input, context.cancellationSignal, turnEnded))
    },
    async sealStep(input) {
      return withHeartbeat(() => drains.sealDrain(input, Context.current().cancellationSignal))
    },
  }
}
