...
#
# Needs: bun, the temporal CLI, GNU coreutils (`timeout`), and an OpenAI key. Nothing here is a
# unit test; it is the evidence for a claim that only shows up across processes.
#
...
RUN="${RUN_DIR:-${TMPDIR:-/tmp}/opencode-l3}"
...
for _ in $(seq 1 30); do
  temporal operator cluster health --address "127.0.0.1:$PORT_TEMPORAL" >/dev/null 2>&1 && break
  sleep 1
done
