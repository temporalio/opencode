...
# The model catalog, fetched once here and mounted into every container. Without it each container
# fetches it itself, with a ten-second timeout, and a catalog host having a slow afternoon reads as
# the session going wrong. Reuses whatever is already cached on this machine when the fetch does
# not come back.
export MODELS_JSON="${MODELS_JSON:-$(mktemp "${TMPDIR:-/tmp}/opencode-models.XXXXXX")}"
if [ ! -s "$MODELS_JSON" ]; then
  curl -fsS --max-time 120 "${OPENCODE_MODELS_URL:-https://models.opencode.ai}/api.json" -o "$MODELS_JSON" \
    || cp "${HOME}/.cache/opencode/models.json" "$MODELS_JSON" 2>/dev/null \
    || { echo "could not get the model catalog: fetch it to a file and pass MODELS_JSON"; exit 1; }
fi

# A workspace link pointing at a package the store does not hold. `bun install` leaves these behind
# when node_modules was pruned by hand, and the first command that needs the package fails in a way
# that reads like its own bug. Cheap to ask here, and the answer is always to install from a clean
# tree.
dangling=""
...
# Built on every run. The compose file mounts `core` and `temporal` over the image, so everything
# else a worker runs is whatever the image was built from, and a stale image runs current source
...
# The project is a directory under the mounted volume rather than the mount point itself, so a host
# that finds a call never came back can move it out of the way instead of only refusing it.
$COMPOSE exec -T serve sh -c \
  'mkdir -p /work/project && cd /work/project && git init -q && echo hello > README.md && git add -A && git commit -qm init' \
  >/dev/null 2>&1 && ok "the project is a git checkout on worker A" || { bad "the project fixture failed"; exit 1; }

new_session() {
  api -X POST http://127.0.0.1:4096/api/session -H 'content-type: application/json' \
    -d '{"directory":"/work/project"}' | sed -n 's/^{"data":{"id":"\([^"]*\)".*/\1/p'
}
prompt() {
...
stops() {
  ...
  case "$body" in *InvalidRequestError*) echo "  history rejected: $body" >&2; return 1 ;; esac
  printf '%s' "$body" | grep -o '"finish":"stop"' | wc -l | tr -d ' '
}
await_turn() {
  local before=$2 now
  for _ in $(seq 1 90); do
    now=$(stops "$1") || return 1
...
before=$(stops "$sid" || echo 0)
prompt "$sid" '"Use the bash tool to run exactly: echo TRAVELLED > /work/project/note.txt && cat /work/project/note.txt. Report the output."'
...
empty=$($COMPOSE exec -T worker-b sh -c 'ls -A /work/project 2>/dev/null | wc -l' 2>/dev/null | tr -d '\r ')
...
before=$(stops "$sid" || echo 0)
prompt "$sid" '"Use the bash tool to run exactly: cat /work/project/note.txt. Report exactly what it printed."'
...
landed=$($COMPOSE exec -T worker-b sh -c 'cat /work/project/note.txt 2>&1' 2>/dev/null | tr -d '\r')
case "$landed" in
