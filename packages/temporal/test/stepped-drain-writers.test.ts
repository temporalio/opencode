...
const drainsOver = (
  ...
  makeSteppedDrains({
    inSession: makeInSession({
      ...
      // The location resolves to a runner and nothing else: what is under test is the bracket
      // around the call, not what the tool does inside it.
      locations: {
...
it.live("marks the directory while a tool call runs and takes the mark back after it", () =>
  Effect.gen(function* () {
    ...
    const { toolCallDrain } = drainsOver(worktrees, directory, {
      runToolCall: ((input: { call: { id: string } }) =>
        Effect.gen(function* () {
