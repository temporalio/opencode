// Deterministic unit tests for the stepped turn body (src/stepped-turn.ts) driven by fake activities --
...
import { isHaltFailure, makeSteppedTurn, type SteppedActivities } from "../src/stepped-turn"
...
const SEALED: StepDrainResult = { continue: true, step: 3, promotion: "steer" }
...
describe("stepped turn", () => {
  ...
  it("reports the calls it did not settle, and says nothing when they all settled", async () => {
    ...
    const log = (message: string, attributes: Record<string, unknown>) => lines.push({ message, attributes })
  ...
  // tree without the other's work. Serial is what moving files between hosts costs; when nothing is
  // moving, the calls still overlap, which is the case the split was measured on.
  for (const serial of [true, false]) {
    it(`runs a step's tools ${serial ? "one at a time when told to" : "together when it is not"}`, async () => {
      let inFlight = 0
      ...
      await makeSteppedTurn({ activities, isCancellation, isHalt, serial })(INPUT)
      expect(tools).toHaveLength(3)
      expect(overlapped).toBe(!serial)
    })
  }
  ...
  it("closes an interrupted step without letting it ask for another", async () => {
    ...
    // out all the same, or a stop landing here publishes no step event and a follower waiting on the
    // turn hangs. What the seal must not do is decide the turn keeps going.
  ...
  it("closes a halted step without carrying on past the refusal", async () => {
    ...
    // still what ends the turn, and the seal is told not to continue, which would otherwise let the
...
describe("halt predicate", () => {
  ...
  const throwsFrom = <A>(body: Effect.Effect<A, unknown, never>, options?: { declineIsInterrupt: true }) =>
